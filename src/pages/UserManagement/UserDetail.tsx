  ...
  if (!user) {
    return (
      <div className="p-8 text-center text-gray-500">
        <p>User not found.</p>
        <button onClick={() => navigate('/users')} className="mt-3 text-sm text-[#2D7A3A] hover:underline">
                  ...
                  {(['Active', 'Inactive'] as const).map((s) => (
                    <button
                      ...
                      className={`w-full flex items-center gap-2 px-4 py-2.5 text-sm text-left hover:bg-gray-50 transition-colors
                        ${user.status === s ? 'font-semibold text-gray-900' : 'text-gray-600'}`}
                    >
                      <span className={`w-2 h-2 rounded-full ${s === 'Active' ? 'bg-green-500' : 'bg-red-500'}`} />
                      {s}
                      {user.status === s && <span className="ml-auto text-[#2D7A3A]">✓</span>}
                    </button>
                  ))}
                </div>
              )}
